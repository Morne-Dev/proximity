...
async function predictObjects() {
  ...
  predictions.value.forEach(prediction => {
    ...
    // Calculate bounding box coordinates relative to video frame
    const scaledX = (x / videoElement.videoWidth) * canvas.width;
    const scaledY = (y / videoElement.videoHeight) * canvas.height;
    const scaledWidth = (width / videoElement.videoWidth) * canvas.width;
    const scaledHeight = (height / videoElement.videoHeight) * canvas.height;
    ...
    // Draw label text
    context.fillText(label, scaledX, scaledY - 10);
  });
